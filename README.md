...
Python版・LINEを基準実装として、文章・写真・音声・動画・ファイルを保存し、同種コンテンツを返信します。対象17プラットフォームは共通契約に登録され、認証なしのオフライン検証が可能です。Telegram、Discord、Misskey、Mastodon、Blueskyはネイティブアダプター、それ以外は設定可能なHTTPアダプターを利用します。
