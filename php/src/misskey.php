...
{
    if ($baseUrl === '' || $token === '') throw new InvalidArgumentException('Misskey base URL and token are required'); foreach (array_slice($reply['messages'] ?? [], 0, 5) as $message) { $body = ['i' => $token, 'text' => $message['text'] ?? '', 'replyId' => $event['reply_to_id'] ?? null]; if (($message['type'] ?? 'text') !== 'text') { if (empty($message['media_url'])) throw new InvalidArgumentException('Misskey media reply requires file ID'); $body['fileIds'] = [$message['media_url']]; } $ch = adapter_curl_init(rtrim($baseUrl, '/').'/api/notes/create'); curl_setopt_array($ch, [CURLOPT_POST => true, CURLOPT_RETURNTRANSFER => true, CURLOPT_HTTPHEADER => ['Content-Type: application/json'], CURLOPT_POSTFIELDS => json_encode($body, JSON_THROW_ON_ERROR)]); $result = curl_exec($ch); $status = curl_getinfo($ch, CURLINFO_RESPONSE_CODE); if ($result === false || $status < 200 || $status >= 300) throw new RuntimeException('Misskey API request failed'); curl_close($ch); }
}
