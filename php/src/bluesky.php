...
{
    if ($serviceUrl === '' || $jwt === '' || $repo === '') throw new InvalidArgumentException('Bluesky service URL, JWT and repo are required'); foreach (array_slice($reply['messages'] ?? [], 0, 5) as $message) { if (($message['type'] ?? 'text') !== 'text') throw new InvalidArgumentException('Bluesky Blob upload is required before media replies'); $record = ['$type' => 'app.bsky.feed.post', 'text' => $message['text'] ?? '', 'createdAt' => gmdate('Y-m-d\TH:i:s.v\Z')]; if (!empty($event['reply_to_uri']) && !empty($event['reply_to_cid'])) $record['reply'] = ['root' => ['uri' => $event['reply_to_uri'], 'cid' => $event['reply_to_cid']], 'parent' => ['uri' => $event['reply_to_uri'], 'cid' => $event['reply_to_cid']]]; $ch = curl_init(rtrim($serviceUrl, '/').'/xrpc/com.atproto.repo.createRecord'); curl_setopt_array($ch, [CURLOPT_POST => true, CURLOPT_RETURNTRANSFER => true, CURLOPT_HTTPHEADER => ['Authorization: Bearer '.$jwt, 'Content-Type: application/json'], CURLOPT_POSTFIELDS => json_encode(['repo' => $repo, 'collection' => 'app.bsky.feed.post', 'record' => $record], JSON_THROW_ON_ERROR)]); $result = curl_exec($ch); $status = curl_getinfo($ch, CURLINFO_RESPONSE_CODE); if ($result === false || $status < 200 || $status >= 300) throw new RuntimeException('Bluesky API request failed'); curl_close($ch); }
}
