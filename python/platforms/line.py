...
class LineAdapter(PlatformAdapter):
    """LINE Messaging API adapter for normalized webhook payloads and push replies."""
    ...
    def parse_event(self, payload: Any, headers=None) -> InboundEvent:
        ...
        source = event.get("source", {}); return InboundEvent(platform="line", user_id=str(source.get("userId") or payload.get("user_id")), content_type=kind, text=message.get("text"), media_url=message.get("id"))
    def send_reply(self, event: InboundEvent, reply: OutboundReply) -> None:
        messages = [{"type": "text", "text": message.text or message.media_url or ""} for message in reply.messages]
        response = self.session.post("https://api.line.me/v2/bot/message/push", headers={"Authorization": f"Bearer {self.access_token}"}, json={"to": event.user_id, "messages": messages}, timeout=30); response.raise_for_status()
