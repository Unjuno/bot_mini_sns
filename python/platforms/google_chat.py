...
class GoogleChatAdapter(PlatformAdapter):
    ...
    def parse_event(self, payload: Any, headers=None) -> InboundEvent:
        ...
        return InboundEvent(platform="google_chat", user_id=str(sender.get("name") or payload.get("user_id")), content_type="file" if media else "text", text=message.get("text"), media_url=media)
    def send_reply(self, event: InboundEvent, reply: OutboundReply) -> None:
        space = os.getenv("GOOGLE_CHAT_SPACE") or event.user_id
