...
class WhatsAppAdapter(PlatformAdapter):
    ...
    def parse_event(self, payload: Any, headers=None) -> InboundEvent:
        ...
        kind = message.get("type", "text"); kind = kind if kind in ("text", "image", "audio", "video", "file") else "text"
    ...
    def send_reply(self, event: InboundEvent, reply: OutboundReply) -> None:
        ...
        for message in reply.messages:
            body = {"messaging_product": "whatsapp", "to": event.user_id, "type": "text", "text": {"body": message.text or message.media_url or ""}}
