...
- [ ] GitHubのリポジトリVisibilityをPublicへ変更したことを確認する（公開操作後に確認）

実サービスの認証情報がない場合でも、認証なしの共通契約テストまでは実行できます。実サービス接続の未確認項目は、公開後もREADMEやIssueで明示してください。公開リポジトリ化はソースコードの公開を意味し、全プラットフォームの本番接続確認を意味しません。
