...
全17プラットフォームは `python/platforms/catalog.py` の共通契約に登録され、`create_adapter(name, offline=True)` で認証なしの検証ができます。Pythonには各プラットフォームの受信・返信経路があり、他の3言語にも同じプラットフォーム群のアダプターがあります。Mastodonはメディアアップロード、Blueskyは画像・動画Blobアップロードまで各言語版に実装しています。ただし、全サービスで全メディア種別の実サービス送受信が完了しているわけではありません。実装状況は照合表を確認してください。
