...
LINEを基準実装として利用し、Telegram、Discord、Zulipなどは同じ共通アダプター契約で追加する。
...
3. **LINEアダプターを python/platforms/line.py へ分離**（実装済み）
   - LINE固有の処理（署名検証、メディア取得、返信形式変換）を移す
   - `python/platforms/base.py` にアダプター契約を定義

4. **Telegramアダプター追加**（実装済み）
   - 共通インターフェースが正しく機能することを確認
   - Telegram用のテストを追加

5. **Discord、Zulip、その他の対象アダプター**（Python実装済み。認証情報は利用者が設定）
...
翻訳と広告は現行仕様および直近の実装対象外とする。将来再検討する場合は、固定フローを変更しない拡張レイヤーとして設計する。
...
現行仕様ではOptional Extensionsを無効にする。現在のユーザー体験は「送るだけ」に保つ。
...
本体は拡張の内部を参照しない。拡張を追加・交換・削除しても、現行仕様の保存と返信は成立するようにする。
