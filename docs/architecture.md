...
LINE、Telegram、Discord、Zulipなど複数のサービスを、同じ投稿システムの入口として利用する。
...
将来構想のうち、現時点で追加対象とするのは翻訳と広告だけとする。共通の固定フローを置き換えず、返信直前の拡張レイヤーとして追加する。
...
MVPではOptional Extensionsを無効にする。将来有効にできる拡張は翻訳と広告だけとし、現在のユーザー体験は「送るだけ」に保つ。
